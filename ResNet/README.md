...
|layer    | Model       | Acc.        |paper|
| ----------------- | ----------- |---|--|
|20| ResNet      | 88.66% (89.71%)      |91.25%|
|  | plain       | 86.79%      |
|32| ResNet      | 91.24% (90.79%)     |92.49%
|  | plain       | 88.33%      |
|44| ResNet      | 90.23% (91.02%)     |92.83%
|  | plain       | 85.18%      |
|56| ResNet      | 90.14% (91.24%)     |93.03%
|  | plain       | 84.38%      |
...
- 9.10
  - 重大错误：没有将训练和测试分离，`model.train()` and `model.eval()`
...
这个问题仍然~~悬而未决~~(见下面9.9分析)，需要通过重复实验来验证

9.8 同样把transform放到了`__getitem__`方法中后，与应用库函数训练效果相仿。(acc: 0.8971)

9.9 晚上躺着，突然醍醐灌顶，我才反应过来，自己原来的操作完全没有用。如果把transform放到了`__getitem__`方法中后，那数据将**在每一次训练的epoch中施以随机的变换**，而之前我以为只需要对数据做一次增强处理，而且还奇怪这样不增加训练的样本，真的会有效果吗？现在解释的通了，这样处理可以防止过拟合。

![trans](img/trans_com__acc.png)

可以看到，如果在**初始化init**时将所有的数据进行了所谓的*增强处理*，结果是和没有增强处理几乎相同，训练数据都发生了过拟合，而且由于测试数据没有进行同样的处理，意味着训练数据信息发生了丢失，导致正确率整体比不增强处理还要差；而在**每次取数据getitem**时进行*增强处理*，将会使过拟合大大抑制。在图中可以看到，随着epoch逐渐增多，在末期训练集的正确率才渐渐逼近100%。
